...
//			ADC1->ISR |= ADC_ISR_OVR;	//при отладке могу ловить оверран todo завести счетчик на всякий случай
	...
	else	//если счетчик считает вверх, то считаем ШИМ
	{
		 ...
		 */
		calcAmplitude();	//вычисление амплитуды
		TIM1->ARR = g_arr1[ARR];				//загружаем регист переполнения
		TIM1->CCR1 = calcPhase(g_arr1[ANGLE]);//вычисление ШИМ для трех фаз, с учетом сдвига фаз
		if (g_arr1[INV_DIR])	//DIR определяет направление вращения. меняет две фазы местами
		{
			TIM1->CCR2 = calcPhase(g_arr1[ANGLE] + PHASE_SHIFT);
			TIM1->CCR3 = calcPhase(g_arr1[ANGLE] + PHASE_SHIFT + PHASE_SHIFT);
		}
...
uint32_t calcAmplitude(void)
{
	/*
	 * Возможные варианты:
	 * для частот до 2 Гц одна прямая
	 * для частот до 20 Гц другая
	 * для всего остального третья
	 * такие огромные числа ради ухода от операций с плавающей точкой, хз насколько это оправдано
	 */
	uint32_t amplitude_tmp = 0;
	if (g_arr1[F_SET] < 20)
	{
		amplitude_tmp = 1500 * g_arr1[F_SET] / 1000;
	}
	else if (g_arr1[F_SET] < 200)
	{
		amplitude_tmp = (211 * g_arr1[F_SET] + 25778) / 1000;
	}
	else
	{
		amplitude_tmp = (107 * g_arr1[F_SET] + 46667) / 1000;
	}

	if (amplitude_tmp > 100) //проверка на выход за пределы
	{
		amplitude_tmp = 100;
	}
	//todo сделать компенсацию просадок по напряжению
	g_arr1[AMPLITUDE] = amplitude_tmp;
	return 0;
}
